...
<section class="page">
  ...
  <p>メルカリ・ヤフオク・ラクマ・Amazon は自動取得できないため、検索ページを開いて見た価格を入力します。</p>
  ${figure(s.shortcuts, '検索ショートカット。チェックしたサイトを「まとめて開く（画面分割）」で並べて表示します。')}
  <ol>
    <li>検索後、「検索ショートカット」の <b>まとめて開く（画面分割）</b> を押すと、チェックしたサイトの検索ページが別ウィンドウで画面いっぱいに並んで開きます。</li>
    <li>初回はブラウザがウィンドウをブロックすることがあります。アドレスバー右端のポップアップのアイコンから「許可」を選び、もう一度押してください。</li>
    <li>各サイトで見た価格を、相場一覧の該当する行の「見た価格」に入力して「＋」を押します。同じ目盛りの横棒に並び、比べられます。</li>
    <li>入力した価格は「手動追加」のカードとしても一覧に入り、比較ボードに追加できます。</li>
  </ol>
