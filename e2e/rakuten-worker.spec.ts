...
async function rakutenSearch(page: Page, query: string) {
  ...
  await expect(page.getByText('楽天市場モード — 検索すると接続します')).toBeVisible();
...
for (const { query, status, message } of fallbackCases) {
  test(`障害: ${query.split(' ')[0]} は理由を表示して見本データに切り替え、実データと誤認させない`, async ({ page }) => {
    ...
    // 検索語に障害シナリオの目印が入っているため見本データは0件になり、見本にある語を案内する
    await expect(resultCards(page)).toHaveCount(0);
    await expect(page.getByText(/見本データには「PS5」「ウォークマン」「3DS」などの候補があります/)).toBeVisible();
    await expect(page.getByText(`直近の検索結果: ${status}`)).toBeVisible();
  });
}

test('キー未設定: 見本データのカードには必ず「見本データ」バッジが付き、CSV でも実データと区別される', async ({ page }) => {
  ...
  for (let i = 0; i < count; i += 1) {
    await expect(resultCards(page).nth(i).getByText('見本データ（実在しない商品）')).toBeVisible();
  }
  await resultCards(page).first().getByRole('button', { name: '比較に追加' }).click();
  await expect(comparedCards(page).first().getByText('見本データ（実在しない商品）')).toBeVisible();
  const { text } = await downloadCsv(page);
  const rows = parseCsv(text.replace(/^\uFEFF/, ''));
...
test('Worker 不在（静的ホスティングで /api が HTML の 404）でも落ちず、設定前として案内する', async ({ page }) => {
  ...
  await rakutenSearch(page, 'PS5');
  await expect(page.getByText(/楽天市場との連携がまだ設定されていないため/)).toBeVisible();
  await expect(resultCards(page).first().getByText('見本データ（実在しない商品）')).toBeVisible();
});

test('障害: 楽天の応答が遅い（8秒超）とタイムアウトとして見本データに切り替える', async ({ page }) => {
  test.slow();
  await page.goto('/');
  await selectDataSource(page, 'rakuten_mock');
...
test('検索語エラー: 楽天が受け付けない語は見本データを出さず、検索語の直し方を案内する', async ({ page }) => {
...
test('オフライン: 通信できないときは理由を表示して見本データに切り替える', async ({ page, context }) => {
  await page.goto('/');
  await selectDataSource(page, 'rakuten_mock');
  await context.setOffline(true);
  await page.getByLabel('商品名・型番・JAN・URL').fill('PS5');
  await page.getByRole('button', { name: 'まとめて探す' }).click();
  await expect(page.getByText(/楽天市場に接続できなかったため/)).toBeVisible();
