...
**Q: 価格は自動で取得されますか？**  
A: 現在のデモ版はサンプルデータです。公式API接続（楽天市場等）は別途設定が必要です。詳しくは `docs/first-official-api-plan.md` を参照してください。

**Q: APIキーはどこに入れますか？**  
A: フロントエンドには入れません。Cloudflare Workers や Vercel Edge Functions などのサーバーサイドに環境変数として設定します。
