...
v0.1 は **デモ版** です。サンプルデータと楽天APIモックで動作するため、APIキーやサーバー設定は不要です。
...
- [ ] アプリが正常に表示される
- [ ] 画面上部に `v0.1 Demo` と実API未接続の注意文が表示される
...
- v0.1 Demo では API キー不要で、そのままデプロイ可能です。
- 楽天などの実API接続は、必要に応じて次フェーズの有償カスタマイズとして追加します。
