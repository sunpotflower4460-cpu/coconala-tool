...
| **かんたん公開（静的版 `app-static/`）** | ❌ 自動取得なし（価格の貼り付け・入力で比較） | Cloudflare などのアカウントだけ | まず試したい・楽天以外で使う |

どちらでも、価格の貼り付け・手動追加・比較・利益計算・CSV・履歴はすべて使えます。
...
- この時点では楽天のキーが無いため、検索すると「取得できず（連携の設定前）」と理由が表示されます（実在しない商品は表示しません）。
...
3. 「利用情報」の「データ使用目的」（必須）に、使い方を書きます（例: 「物販リサーチ用の相場比較ツールで、利用者が入力した商品名・型番・JANコードで楽天市場の商品を検索し、商品名・価格・画像・商品ページへのリンクを一覧表示するために使用します。取得したデータは保存・再配布しません。」）。
   「必要とされるQPS（1秒あたりの必要量）」は `1` で足ります（1回の検索で楽天へは1回だけ問い合わせます）。
4. 「APIアクセススコープ」は **「楽天市場API」だけ**にチェックを入れて登録し、「詳細を見る」で「アプリケーションID」と「アクセスキー」を控えます（トラベル・ブックス等は使いません）。
5. アプリには**有効期限（登録から約1年）**があります。期限が切れると「楽天市場の設定が正しくない」と表示されるので、
...
- 検索すると「この版は自動取得なし」と表示され、商品カードは出ません。相場一覧から検索ページを開き、貼り付け・入力で価格を並べます。
...
- 楽天の応答は約8秒でタイムアウトし、失敗しても画面は落ちずに理由を表示します（実在しない商品は出しません）。
