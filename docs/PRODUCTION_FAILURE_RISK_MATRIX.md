...
対象: 相場カード比較ボード (`coconala-tool`) v1.0.0-beta.2
...
## DESK-08 取り込みで前の検索・別のページの値段が混ざる

- 重大度: P1
- 状態: Protected
- 故障: 取り込み中に新しく検索すると前の検索の値段が新しい一覧に入る。右のタブで商品ページに移ったまま取り込むと、関連商品の値段が入る。新しい検索ページに切り替わる前の、前の検索ページを読む
- 対策: 検索中は取り込めない。取り込み結果は開始時と同じ検索のときだけ反映。検索結果のページ（同じ場所・道筋）以外と、新しい検索ページに切り替わる前のタブは読まず、戻し方を案内
- 自動テスト: `e2e-desktop/desktop.spec.ts`（商品ページ等に移っているサイト）
