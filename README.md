...
物販・せどり向けの相場リサーチ補助ツールです。楽天市場の公式APIによる価格候補、他サイトへの検索リンク、
手動追加した商品を一つのボードで比較し、利益見込み・履歴・CSV出力までまとめて管理できます。

現在のバージョン: **v0.9.0-rc.11**（正式販売前の候補版）
...
| ソース | 状態 | 説明 |
|---|---|---|
| 楽天市場 商品検索API（2026-07-01版） | ✅ 公式API対応 | Cloudflare Workers に `SERVER_RAKUTEN_APP_ID` と `SERVER_RAKUTEN_ACCESS_KEY` を設定すると実データ（`公式API取得`）になります |
| メルカリ・ヤフオク・Amazon・eBay・Yahoo!ショッピング 等 | 検索リンク＋手動追加 | 自動取得はしません。検索リンクを開いて確認し、手動追加する運用です |
...
| 公開方法 | 画面 | 楽天の実データ |
|---|---|---|
| Cloudflare Workers（推奨・正式対応。`npm run deploy`） | ✅ | ✅ |
...
- 商品名・型番・JAN・URL で「まとめて探す」→ 画像つき価格カード（出どころラベル付き）
...
Vite / React / TypeScript / Tailwind CSS / Zustand / localStorage / Cloudflare Workers（`/api/rakuten`）
