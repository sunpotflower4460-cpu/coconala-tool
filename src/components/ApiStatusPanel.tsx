...
const notes = [
  ...
  '接続できないサイトがあるときは理由を表示します。実在しない商品（見本）を代わりに表示することはありません。',
...
const currentSourceLabel: Record<DataSourceMode, string> = {
  rakuten_mock: IS_STATIC_BUILD
    ? '楽天市場（この版は自動取得なし・貼り付け／手入力で比較）'
    : '楽天市場（設定済みなら実データ。未設定・接続できない時は理由を表示）',
  multi: IS_STATIC_BUILD
    ? '楽天・Yahoo!ショッピング・eBay（この版は自動取得なし・貼り付け／手入力で比較）'
