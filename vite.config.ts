...
//  - `--mode desktop`: デスクトップアプリ（Electron）の画面。/api/* はアプリ本体（electron/main.ts）が処理する。
// Vitest は jsdom 上のテスト専用のため Cloudflare プラグイン（workerd）は読み込まない。
export default defineConfig(({ mode }) => {
  const isVitest = Boolean(process.env.VITEST);
  if (mode === 'e2e') process.env.CLOUDFLARE_ENV = 'e2e';
  const useCloudflare = !isVitest && mode !== 'desktop';

  return {
    plugins: [react(), ...(useCloudflare ? [cloudflare()] : [])],
    define: {
      'import.meta.env.VITE_DEPLOY_MODE': JSON.stringify(mode === 'static' ? 'static' : mode === 'desktop' ? 'desktop' : 'worker'),
    },
    test: {
      environment: 'jsdom',
      setupFiles: ['./vitest.setup.ts'],
      // e2e/ と scripts/ は Playwright（別ランナー）専用のため vitest の対象から除外する。
      exclude: ['**/node_modules/**', '**/dist/**', '**/dist-desktop/**', 'e2e/**', 'e2e-desktop/**', 'scripts/**'],
    },
  };
});
