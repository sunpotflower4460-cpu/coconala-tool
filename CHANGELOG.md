...
## v1.0.0-beta.2 — 楽天の登録項目の追加と細かな不具合の修正

- 楽天の登録画面の「データ使用目的」「必要とされるQPS」「APIアクセススコープ（楽天市場APIだけ）」を、設定画面・マニュアル・公開ガイドに追加（コピーボタンつき）
- 取り込み中に新しく検索しても、前の検索の値段が混ざらないように（検索中は取り込みボタンを押せない）
- 右のタブで商品ページ等に移っているサイト・読み込み途中のサイトは取り込まず、「検索結果に戻してから」と案内
- 最初の検索で、表示中のタブのページがすぐ出るように。ウィンドウを閉じたら右のタブのページも止める
- 画面の拡大縮小・再読み込み・開発者ツールのメニューを外す（右のタブの位置がずれるため）
- 右のタブのページから既定のブラウザを開くのは https・1秒に1回までに。許可していないサイトへの転送を止める
- キーの保存に失敗したときに「保存してテスト」が止まったままにならないように。取り込みのお知らせの見出し・保存場所の説明を実際の状態に合わせる
- タブを ← → で切り替えられるように。設定ダイアログの外へ Tab で移らないように
- 利用規約にデスクトップアプリ（インストーラーの再配布禁止・取り込み機能の利用責任・キーの保管）を追記

## v1.0.0-beta.1 — デスクトップアプリ（ボタン1つで7サイトを並べる）

- Windows / Mac のデスクトップアプリ（Electron）を追加。販売の主役をデスクトップアプリに
- 「まとめて探す」1回で、楽天市場・Yahoo!ショッピング・eBay を画像つきで一覧表示（1サイト最大20件・安い順）し、
  メルカリ・ヤフオク・ラクマ・Amazon の実際の検索ページをアプリ右側のタブに開く
- 「値段を取り込む」1回で、右側に表示中の検索ページ（各サイト1ページ）から値段と商品リンクだけを一覧・相場一覧に取り込む
  （利用者の操作時のみ・画像や説明文は読まない・ページ送りや自動巡回はしない・サイトごとにオフにできる）
- 検索直後に「サイトごとの状況」を1行で表示（どのサイトの結果がどこに出ているか・キー未設定なら押して設定）。色の凡例に頼らず文で説明
- はじめての設定ウィザード: 登録ページで入力する内容をコピーボタンつきで案内し、貼ったキーをその場でテスト。キーは OS の暗号化保管でパソコン内に保存
- 楽天アプリの有効期限を登録すると、30日前からアプリ内でお知らせ
- 納品物を5ファイル（Windows 用・Mac 用×2・マニュアル・詳しい資料 ZIP）に。各200MB未満（ココナラのトークルームで送れる大きさ）
- マニュアルをデスクトップアプリの実画面（番号札つき）で作り直し。インストール時の確認画面の開き方・困ったときの表を追加
- CI: macOS で全検証（インストーラー作成・作ったアプリの起動確認を含む）、Windows でインストーラー作成・起動・デスクトップ版 E2E
