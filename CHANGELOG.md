...
## v0.9.0-rc.15 — 実在しない商品（サンプル・見本）を表示しない

- 固定の「サンプルデータ」（データソースの選択肢）と、接続できないときの「見本データ」を廃止。紛らわしい実在しない商品は一切表示しない
- 接続できないとき・パソコン内（1ファイル版）・かんたん公開版では、商品カードの代わりに理由と
  「相場一覧から検索ページを開き、貼り付け・入力で価格を並べる方法」を表示。ヘッダーは「自動取得できませんでした／この版は自動取得なし — 貼り付け・手入力で比較」
- お試しボタンは「Nintendo Switch 2 で試してみる」に統一
- 旧版で保存した「サンプルデータ」の選択は「まとめて」として読み込む（古い履歴のカードはそのまま表示）
- マニュアル・README・公開ガイド・出品文を更新

## v0.9.0-rc.14 — 初心者でも ZIP から迷わず開ける

- 納品ZIPを開いて最初に見えるのを「①ツールを開く.html」「②マニュアル.pdf」「③詳しい資料（公開する人向け）」の3つだけに整理
- ①: ダブルクリック（file://）でそのまま動く1ファイル版（JS・CSS・アイコンを埋め込み）。Windows で ZIP を展開せずに開いても動く
- 最初の画面に「PS5 で試してみる」ボタン（入力と検索を自動で行う）
- マニュアルに「0. まず開いてみる（Windows / Mac）」を追加。README_FIRST・納品物の説明を新しい構成に更新
- 検索処理を共通化（検索ボタン・Enter・お試しボタン）

## v0.9.0-rc.13 — ツールの横に並べて、貼り付けで一括取り込み

- 「まとめて開く」を、ツールを画面の左 1/3 に残して右 2/3 に各サイトを格子状に並べる配置に変更
- 相場一覧の各行の「開く」も同じ位置のウィンドウで開き、押し直しても同じウィンドウを使い回す
- 貼り付けで一括取り込み: メルカリ・ヤフオク・ラクマ・Amazon の検索ページを「すべて選択→コピー」して貼ると、
  価格をまとめて相場一覧に並べる（クーポン・ポイント・送料の金額を除外。「検索表示から推定」として表示。サイトへの自動アクセスはしない）
- マニュアル PDF の「メルカリ・ヤフオク等の価格も並べて比較する」を新しい手順と画面写真に更新

## v0.9.0-rc.12 — 楽天・Yahoo!・eBay のまとめて検索と相場一覧

- データソース「まとめて（楽天・Yahoo!・eBay）」: 3サイトの公式APIを同時に検索して1つの一覧に並べる。
  一部のサイトが失敗しても他のサイトの実データは表示し、失敗したサイトは理由だけ表示（見本データを混ぜない）
- Yahoo!ショッピング 商品検索API（v3）と eBay Browse API（OAuth アプリ用トークンを Worker で保持）のサーバー側プロキシを追加
- 相場一覧: サイト別の価格帯を同じ目盛りの横棒で並べ、最安のサイトと中央値を表示。eBay はドル円レートで円換算
- メルカリ・ヤフオク・ラクマ・Amazon は、見た価格を相場一覧に入力すると同じ目盛りで並ぶ（自動取得・埋め込みはしない）
- 検索ショートカットに「まとめて開く（画面分割）」: チェックしたサイトを別ウィンドウで格子状に並べて開く。Amazon を追加
- 検索結果のサイト別絞り込みと、円換算での安い順・高い順の並べ替え
- 手動追加で URL を貼るとサイト名を自動入力して価格欄へ進む
- Yahoo! JAPAN Web API のクレジット表記を追加。レート制限を 1IP 60秒60回（まとめて検索で約20回）に
- 相場から大きく外れた価格（中央値の35%未満・3倍超。付属品・まとめ売り等）を相場一覧の価格帯から自動で除き、カードに注意を表示（切り替え可）
- 画面写真入りの操作・導入マニュアル（`マニュアル.pdf`・13ページ）を毎回自動で撮影・生成して納品ZIPに同梱
- 手動追加ダイアログの背景をほぼ不透明にして、入力欄を読みやすく
